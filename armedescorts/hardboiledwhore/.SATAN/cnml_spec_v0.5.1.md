...
# PÂTE (Positional Affine Text Encoding) Specification
...
SPAN :=
  "{{"
    SOURCE_SPAN_ID?
    SOURCE_ALIGN_OP
    ( TARGET_ALIGN_OP TARGET_SPAN_ID | TARGET_SPAN_ID TARGET_ANCHOR )
    OFFSET?
    ("::" CONTENT)?
  "}}"
...
OFFSET_EXPRESSION := SIGN RATIONAL_NUMBER SPAN_WIDTH_REFERENCE
SIGN := "+" | "-"
RATIONAL_NUMBER := POSITIVE_INTEGER "/" POSITIVE_INTEGER | POSITIVE_INTEGER
...
ID_START := Unicode_Letter
ID_CONTINUE := Unicode_Letter | Unicode_Mark | Unicode_Number
...
1. bind A.trailing → B.center
2. inherit axis = B.width vector
3. apply +C.width along axis

### Metric Operator (Intrinsic Field)
