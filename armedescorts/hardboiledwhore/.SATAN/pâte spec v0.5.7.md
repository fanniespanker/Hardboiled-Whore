...
```
{{ SOURCE_ID? SOURCE_ALIGN TARGET_ALIGN TARGET_ID OFFSET? :: CONTENT }}
```
...
Alignment operators are syntactic selectors over the fixed anchor set A = {<, |, >}.
They do not represent anchors or nodes.
They resolve at compile-time into ordered anchor pairs used to construct graph edges.
...
### 7.2 Alignment Relation

An alignment operator defines a pair:
...
No additional semantics exist beyond anchor pairing.
...
```
OFFSET := SIGN RATIONAL_NUMBER SPAN_ID
```
...
Offsets define affine displacement:

```
pos(A.anchor) = pos(B.anchor) + δ
```
...
## 10. Evaluation Model (Renderer Responsibility)

The renderer performs deterministic resolution:

### Step 1 — Parse
Extract spans and anchors.

### Step 2 — Build Graph
Convert alignment expressions into edges.
...
No approximation, optimization, or heuristic solving is permitted.

---

## 11. Validity Rules

### 11.1 Determinism Rule
Each constraint MUST produce a single consistent solution.

### 11.2 Conflict Rule
If multiple constraints apply to the same anchor:
- they MUST be identical
- otherwise the graph is invalid

### 11.3 No Relaxation Rule
PÂTE does NOT:
- resolve conflicts
...
This is the complete semantic substrate.

---

## 13. System Positioning

- CNML → structural + narrative representation
- PÂTE → spatial constraint compilation
- Renderer → deterministic evaluation engine

PÂTE has no runtime beyond graph evaluation.
...
> a deterministic affine graph language over span anchors, compiled from CNML and evaluated without optimization or normalization.

Meaning is entirely encoded in graph structure and resolved geometrically, not procedurally.
